...
2. Create a `.env` file (never commit it) with your unlimited account credentials and default filter inputs:
   ```
   LANDINSIGHTS_EMAIL=your-email@example.com
   LANDINSIGHTS_PASSWORD=your-strong-password
   LANDINSIGHTS_COUNTY=Travis County, TX
   LANDINSIGHTS_ACRES_FROM=0
   LANDINSIGHTS_ACRES_TO=10000
   LANDINSIGHTS_TOKEN_BALANCE=500000
   ```
3. Run the automation for the county defined in `.env`:
   ...
   - When exports finish, the CSV files appear under `artifacts/downloads/` with descriptive names.
4. Optional flags:
...
- Set `LANDINSIGHTS_TOKEN_BALANCE` to the number of parcel tokens available in your Land Insights account.
- The automation blocks any export whose parcel count exceeds the remaining balance and logs the tokens consumed after each successful download.
- Leave the variable unset to disable token tracking.

### Sample multiple counties

- `npm run counties:sample` launches `scripts/batchCounties.mjs`, which picks three random counties from `data/texas-counties.json` and runs `loginTrace` once per county.
- Adjustments:
...
- Re-run the script while performing the exact export workflow (county selection, filters, download) to capture new network behavior if Land Insights changes its UI.
- Promote the login flow into a Prefect/Temporal task once the endpoints are mapped, then add job orchestration and storage described in your automation plan.
